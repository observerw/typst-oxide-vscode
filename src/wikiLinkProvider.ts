...
export class WikiLinkProvider implements vscode.DocumentLinkProvider {
    // Regex to match wiki links: [[path/to/file:label]]
    ...
    provideDocumentLinks(
        ...
        while ((match = WikiLinkProvider.WIKI_LINK_REGEX.exec(text)) !== null) {
            ...
            // Create a document link with custom scheme to handle navigation
            const link = new vscode.DocumentLink(
                range, 
                vscode.Uri.parse(`typst-wiki://${targetUri.fsPath}#${encodeURIComponent(label)}`)
            );
            
            link.tooltip = `Navigate to "${label}" in ${PathResolver.getWorkspaceRelativePath(targetUri)}`;
...
export class WikiLinkHandler {
     ...
     */
    static async handleWikiLink(uri: vscode.Uri): Promise<void> {
        try {
            // Parse the custom URI: typst-wiki://file/path#label
            const filePath = uri.path;
            const label = decodeURIComponent(uri.fragment);
            ...
            const success = await LabelSearcher.navigateToLabel(document, label);
            ...
            if (!success) {
                ...
                if (addLabel === 'Add Label') {
                    await this.addLabelToFile(document, label);
                }
            }
            ...
            vscode.window.showErrorMessage(
                `❌ Failed to navigate to wiki link`,
                {
                    modal: false,
     ...
     */
    private static async createNewFile(uri: vscode.Uri, label?: string): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                ...
                {
                    modal: false,
                    detail: label ? `Added label "${label}" as a comment in the new file.` : undefined
                }
            );
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
     ...
     */
    private static async addLabelToFile(document: vscode.TextDocument, label: string): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                ...
                {
                    modal: false,
                    detail: 'The label has been added as a comment at the end of the file.'
                }
            );
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
