...
export class LabelSearcher {
     ...
     */
    static async findLabel(document: vscode.TextDocument, label: string): Promise<LabelSearchResult> {
        ...
        const searchStrategies = [
            ...
            (line: string, lineIndex: number) => this.findHeading(line, lineIndex, label),
            // 2. Exact match as comment (// Label)
            (line: string, lineIndex: number) => this.findComment(line, lineIndex, label),
            // 3. Simple text match
            (line: string, lineIndex: number) => this.findTextMatch(line, lineIndex, label),
        ];
     ...
     */
    private static findHeading(line: string, lineIndex: number, label: string): LabelSearchResult {
     ...
     */
    private static findComment(line: string, lineIndex: number, label: string): LabelSearchResult {
        ...
        if (blockMatch && blockMatch[1].trim().toLowerCase() === label.toLowerCase()) {
     ...
     */
    private static findTextMatch(line: string, lineIndex: number, label: string): LabelSearchResult {
     ...
     */
    static async navigateToLabel(document: vscode.TextDocument, label: string): Promise<boolean> {
