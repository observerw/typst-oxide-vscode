...
export class WikiLinkCompletionProvider implements vscode.CompletionItemProvider {
    ...
    async provideCompletionItems(
        ...
        const partialPath = wikiLinkMatch[1];
        
        // Check if we're looking for labels (after ':')
        if (partialPath.includes(':')) {
            return this.provideLabelCompletions(document, partialPath, token);
        }
        
        // Otherwise, provide file path completions
        return this.provideFilePathCompletions(document, partialPath, token);
    }
    ...
    private async provideFilePathCompletions(
        ...
        try {
            ...
            for (const file of files) {
                ...
                completion.documentation = new vscode.MarkdownString(`Link to \`${pathWithoutExt}\``);
        ...
        try {
            ...
            for (const label of labels) {
                ...
                if (label.text.toLowerCase().includes(partialLabel.toLowerCase())) {
                    ...
                    completion.detail = `${label.type} in ${path.basename(targetUri.fsPath)}`;
     ...
     */
    private async findTypstFiles(searchDir: string, pattern: string): Promise<string[]> {
        const files: string[] = [];
     ...
     */
    private async extractLabelsFromDocument(document: vscode.TextDocument): Promise<Array<{text: string, type: string}>> {
        ...
        for (const line of lines) {
            ...
            // Extract single-line comments (// Label)
